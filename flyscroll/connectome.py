"""Loss-accounted import of the retained MaleCNS v1.0 graph for FlyScroll.

Stores topology and annotations only. Does not infer missing dynamics,
receptors, or doomscrolling behaviour from the wiring diagram.
"""
...
def import_graph(dataset_id: str = "malecns_v1") -> dict:
    ...
    if lock_path.exists():
        ...
        if any(locked[name]["sha256"] != info["sha256"] for name, info in hashes.items()):
            raise ValueError(
                "Source files changed since the lock was created; "
                "use a new versioned dataset directory."
            )
    else:
    ...
    with pa.OSFile(str(temporary), "wb") as sink, ipc.new_file(sink, schema) as writer:
        for number in range(reader.num_record_batches):
            ...
            pre, post, weights = [
                batch.column(batch.schema.get_field_index(c)).to_numpy(zero_copy_only=False)
            ...
            writer.write_batch(
                pa.record_batch([pa.array(i), pa.array(j), pa.array(count)], schema=schema)
            )
    ...
    report = {
        ...
        "upstream_autapses_excluded": config.get("upstream_autapses_excluded", False),
        "additional_edge_strength_threshold": None,
        "synaptic_weights_are_contact_counts": True,
        "source_hashes": hashes,
        "graph": stats,
        "neural_dynamics_validated": False,
        "doomscroll_behavior_validated": False,
        "remaining_gaps": [
            "Receptor-dependent synapse dynamics and neuromodulation",
            "Calibrated retinal dynamics",
            "Validated novelty/habituation readout for video interest",
        ],
    }
    (output / "report.json").write_text(json.dumps(report, indent=2) + "\n")
    public = ROOT / "outputs" / "flyscroll" / "connectome-import.json"
